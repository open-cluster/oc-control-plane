...
An Organization is the tenant boundary. A User holds at most one current Membership,
with an Admin, Editor, or Viewer Role. Authentication resolves that Membership rather
than accepting an Organization during sign-in.

An Integration is one configured source. It currently offers bounded observation/read Tools or accepts
