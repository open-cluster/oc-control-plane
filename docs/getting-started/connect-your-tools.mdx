...
You need the **Admin** role in your Organization and the provider permissions
...
successful Verification sets the Integration to `verified`; individual Tools are offered
only when their verified grants are present. Before the first Verification the status is
empty, and an unsuccessful Verification sets it to `failed`.
