...
- `opencluster-credentials` with keys `bootstrap-token`, `sealing-key`, and `model-key`.

The shipped chart names these Secret keys `bootstrap-token` and `sealing-key`; they supply
