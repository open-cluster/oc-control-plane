...
is already `verified`. Nothing is pasted, and no token passes through your
...
Select **Verify** on the integration. Verification checks the credential and records its
granted scopes. The connected workspace identity comes from the completed connection and
is not editable configuration.

A bot token holding the scopes OpenCluster asks for reports `verified`. Workspace-wide
