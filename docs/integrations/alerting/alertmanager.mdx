...
Alertmanager supplies alert lifecycle context, not runtime or change evidence. Its
grouping key determines provisional Incident grouping and does not prove a shared cause.
Connect Kubernetes, GitHub, or another evidence source when an Investigation needs to
explain the alert.
...
[run the first Investigation](/getting-started/run-your-first-investigation) from a test alert.

## Availability

Prometheus Alertmanager is available in OSS v0.1. Alertmanager and its mark belong to the
Prometheus project. Review the [provider webhook contract](https://prometheus.io/docs/alerting/latest/configuration/#webhook_config).
