...
- Prometheus Alertmanager 0.28.0 or later. Version 0.28.0 added the `http_headers`
  configuration used for the webhook secret; 0.27.0 and earlier reject it. OpenCluster
  tests the configuration below against Alertmanager 0.34.0.
...
<Steps>
  ...
  <Step title="Configure Alertmanager">
    ...
    receivers:
      - name: opencluster
        webhook_configs:
          - url: "https://oc.example.com/webhooks/v1/integrations/<integration-id>/alert-events"
            send_resolved: true
            http_config:
              http_headers:
                X-OpenCluster-Token:
...
Prometheus Alertmanager is available in OSS v0.1. Alertmanager and its mark belong to the
Prometheus project. Review the [provider contract](https://prometheus.io/docs/alerting/latest/configuration/#webhook_config),
[adapter source](https://github.com/open-cluster/oc-control-plane/tree/main/internal/integrations/alertmanager),
...
[Edit this page](https://github.com/open-cluster/oc-control-plane/edit/main/docs/integrations/alerting/alertmanager.mdx)
