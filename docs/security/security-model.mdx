...
Authenticated Organization-scoped API requests send one
`X-OpenCluster-Organization` header. OpenCluster verifies that selector against the
signed-in User's current membership before the request reaches product behavior.
