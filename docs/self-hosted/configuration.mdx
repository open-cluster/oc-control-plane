...
| `OC_BOOTSTRAP_TOKEN_FILE` | One-time first-Admin bootstrap token file. |
...
Organization explicit so a separate cloud composition can select managed or encrypted
Organization BYOK later without changing investigation behavior.
...
30 seconds to write a response, and 60 seconds for an idle connection.
