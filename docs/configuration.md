...
| Investigations | `OC_MODEL_PROVIDER`, `OC_MODEL_NAME`, `OC_MODEL_KEY_FILE`, `OC_MODEL_CONSENTED_PROVIDERS` (+ optional `OC_MODEL_EFFORT`, `OC_MODEL_BASE_URL`, `OC_MODEL_SPEND_CEILING_CENTS`, `OC_INVESTIGATION_WINDOW_LEAD`, `OC_INVESTIGATION_MAX_TOOL_RUNS`, `OC_INVESTIGATION_MAX_TURNS`) |
...
- **The window lead** (`OC_INVESTIGATION_WINDOW_LEAD`, default `2h`) widens every
  ...
  the widened window.
- **The investigation ceilings** (`OC_INVESTIGATION_MAX_TOOL_RUNS`, default 30;
  `OC_INVESTIGATION_MAX_TURNS`, default 20) are positive whole numbers bounding the
  investigator's conversation; a reached ceiling — like spend, wall clock, or
