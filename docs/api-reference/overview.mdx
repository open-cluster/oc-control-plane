---
title: "API overview"
description: "Authenticate, use your Organization, handle errors and pagination, and distinguish product and webhook routes."
---
...
Each authenticated User belongs to one Organization. Read its canonical ID from the
current session:
...
export ORGANIZATION_ID="$(
  curl --fail-with-body --silent \
    ...
    "$OPENCLUSTER_URL/api/v1/session" |
  jq --raw-output '.organization.organizationId'
)"
...
The current API still carries the resolved Organization in its header for Organization-owned
operations. The server verifies it against the User's current Membership; callers cannot
switch Organizations by changing the value.
