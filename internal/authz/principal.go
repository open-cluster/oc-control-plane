...
//
// It lives here rather than in internal/storage because the membership is this package's
...
const (
	...
	KindServiceAccount Kind = Kind(audit.ActorServiceAccount)
...
type Principal struct {
	kind Kind
	// id is the user or service-account identifier. Empty only for the system principal.
	id string
	// displayName is what the record will call this actor. For a person it is the name their
	// identity provider gave; for a service account it is the account's name.
	displayName string
	...
	// credentialID names the session or API token this request presented, so a revocation can
	// be traced to the credential it ended and an audit event can say which token acted.
	credentialID string
...
func NewPrincipal(
	...
	switch kind {
	case KindUser, KindServiceAccount:
