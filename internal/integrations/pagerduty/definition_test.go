...
func TestDefinitionMirrorsTheSeededRow(t *testing.T) {
	...
	if definition.ID != integrations.TypePagerDuty || definition.Key != "pagerduty" {
		t.Errorf("identity = %d %q", definition.ID, definition.Key)
	}
	if definition.Category != integrations.CategoryIncident {
		t.Errorf("category = %q", definition.Category)
	}
	if definition.RequiresRelay || definition.ReceivesWebhooks {
		t.Error("pagerduty is reached outbound with a token; no relay, no webhooks")
	}
...
func TestTheOnlyConfigurationFieldIsTheSecretToken(t *testing.T) {
	...
	if len(definition.Config) != 1 {
		t.Fatalf("config declares %d fields, want the api token alone", len(definition.Config))
	}
	token := definition.Config[0]
	if token.Name != "apiToken" || !token.Secret || !token.Required {
		t.Errorf("apiToken = %+v; it must be required and secret", token)
	}
...
func TestEveryToolDeclaresItsWholeContract(t *testing.T) {
	...
	for _, tool := range Definition(NewClient("")).Tools {
		if tool.Name == "" || !strings.HasPrefix(tool.Name, "pagerduty.") {
