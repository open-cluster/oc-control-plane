...
// The invariant these tests protect is organization isolation: evidence never crosses an
// organization boundary. Every test below is written so that removing
...
func TestBoundary_AJobNamingARelayThatDoesNotServeItsIntegrationIsRefused(t *testing.T) {
	...
	refusal, err := database.EnqueueJob(context.Background(), organization, storage.RelayJob{
		ID:                uuid.New(),
...
func TestBoundary_AJobCannotNameAnotherOrganizationsIntegration(t *testing.T) {
	...
	refusal, err := database.EnqueueJob(context.Background(), one, storage.RelayJob{
		ID:                uuid.New(),
