...
// The limiter bounds what one Connection can spend on every other tenant's behalf. What it
// must never do is let one Connection's traffic shed another's, so the isolation is asserted
...
// The property that matters most: shedding is per Connection. One compromised or
// misconfigured source must not be able to spend another tenant's intake capacity.
func TestLimiter_OneConnectionExhaustedLeavesAnotherUntouched(t *testing.T) {
	t.Parallel()
...
// anyone with many identifiers deny service to every real Connection.
func TestLimiter_IsBoundedInMemoryAndStillAdmitsRealTraffic(t *testing.T) {
	...
	// And a real Connection arriving after that flood is still served rather than refused to
...
func TestLimiter_IdleConnectionsAreForgotten(t *testing.T) {
	t.Parallel()
