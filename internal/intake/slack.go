...
// acceptSlackMessage persists the message and answers. It opens the turn as an unclaimed
// record and does not wait for it.
func (h *surface) acceptSlackMessage(
	...
	// The turn is opened here and CLAIMED elsewhere. Opening it writes a row; running it is
	// the ordinary worker's job, which is what keeps this answer inside Slack's timeout
	// however long the investigation takes.
	//
	// A refusal to open is not a failure of this request. The message is persisted and
	...
	if _, opened, err := h.Placements.OpenTurn(ctx, organization, outcome.Conversation,
		...
		h.Logger.InfoContext(ctx, "a slack message is queued behind a running turn",
			slog.String("org_id", organization.String()),
			slog.String("conversation_id", outcome.Conversation.String()))
	}
