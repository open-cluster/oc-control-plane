	...
	return audited(ctx, p, principal, organization, audit.ActionConversationOpened,
		func(ctx context.Context, transaction pgx.Tx) (
			...
			row := transaction.QueryRow(ctx, `
				INSERT INTO conversation (conversation_id, org_id, episode_id, surface,
				                          subject, created_by)
				VALUES ($1, $2, $3, $4, $5, $6)
			...
			if err != nil {
				if isForeignKeyViolation(err) {
					// The only foreign key on the insert is the episode's, and it is
					// org-composite: an episode from another tenant is not distinguishable
					// here from one that does not exist, which is the answer either way.
					return conversation.Conversation{}, audit.Target{}, nil,
						conversation.ErrEpisodeUnknown
				}
				return conversation.Conversation{}, audit.Target{}, nil,
...
func (p *Placements) ConversationDetail(
	...
	turnRows, err := pool.Query(ctx, `
		SELECT investigation_id, turn, status, answer, stopped_by, error, created_at,
		       concluded_at
		  FROM investigation
...
func openTurn(
	...
	if err := transaction.QueryRow(ctx, `
		SELECT state, episode_id, subject
		  FROM conversation
		 WHERE conversation_id = $1 AND org_id = $2
		   FOR UPDATE`, id, organization.String()).Scan(
		&state, &episodeID, &subject); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
	...
	from, until, err := turnWindow(ctx, transaction, organization, episodeID, lead)
	if err != nil {
		return conversation.Turn{}, false, err
	}
	...
	err = transaction.QueryRow(ctx, `
		INSERT INTO investigation (investigation_id, org_id, episode_id, question,
		...
		investigationID, organization.String(), episodeID, question, subject, from, until,
...
// turnWindow derives the turn's time bounds. An episode-associated conversation reaches
// back before the incident began and forward to now while it is still open; one naming no
// episode gets the lead ending now.
func turnWindow(
	ctx context.Context, transaction pgx.Tx, organization tenancy.Organization,
	episodeID *uuid.UUID, lead time.Duration,
	...
	if err := transaction.QueryRow(ctx, `
		...
		*episodeID, organization.String()).Scan(
		&firstSeen, &lastSeen, &status); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return now.Add(-lead), now, nil
		}
		return time.Time{}, time.Time{}, fmt.Errorf("reading a turn's episode: %w", err)
	}
	until := lastSeen
	if status == 1 {
		until = now
...
func scanConversation(
	...
	if err := row.Scan(&found.ID, &episodeID, &surface, &found.Subject, &state,
