...
func TestSlackEvents_ADirectMessageDoesNotOpenAConversation(t *testing.T) {
	...
	body := `{"type":"event_callback","api_app_id":"A0OPENCLUSTER","team_id":"T0ACME",` +
		`"event_id":"EvDirectMessage","event":{"type":"message","channel":"D0PRIVATE",` +
		`"channel_type":"im","ts":"1700000099.1","user":"U9SRE",` +
		`"text":"why is checkout failing?"}}`
	if status, answer := plane.deliverEvent(t, body); status != http.StatusOK {
		t.Fatalf("a direct message answered %d: %s", status, answer)
	}
	if found := plane.conversationsIn(t); len(found) != 0 {
		t.Fatalf("a direct message opened conversations: %+v", found)
	}
}
	...
	for {
		...
		err = database.QueryRow(ctx, `
			SELECT source_reference
			 ...
			 WHERE org_id = $1 AND provider_channel_id = 'C0INCIDENTS'`,
			surfaceOrg).Scan(&reference)
		if err == nil && reference != "" {
			const want = "https://acme.slack.com/archives/C0INCIDENTS/p1700000007123"
			if reference != want {
				t.Fatalf("source reference = %q, want %q", reference, want)
			}
			var audits int
			if err = database.QueryRow(ctx, `
				SELECT count(*)
				  FROM audit_event
				 WHERE org_id = $1 AND action = 'integration.credential.unsealed'`,
				surfaceOrg).Scan(&audits); err != nil || audits == 0 {
				t.Fatalf("credential unseal audit count = %d, error = %v", audits, err)
			}
			return
		}
		select {
		case <-ctx.Done():
			t.Fatalf("the accepted mention never gained provider provenance: %v", err)
		...
		err = database.QueryRow(ctx, `
			SELECT work.status, message.source_reference
			  ...
			  JOIN conversation_message AS message
			   ...
			   AND message.sequence = work.message_sequence
			 WHERE work.org_id = $1`, surfaceOrg).Scan(&status, &reference)
		if err == nil && status == 5 {
			if reference != "" {
				t.Fatalf("an unavailable permalink produced source reference %q", reference)
			}
			if conversations := plane.conversationsIn(t); len(conversations) != 1 {
				t.Fatalf("the accepted question did not retain its conversation: %+v", conversations)
			}
			return
		}
		select {
