// Package e2e runs the Relay and the control plane as real processes: the protocol proof. A
// job crosses between the two halves and durable truth is asserted. That is what the
// `_test.go` files here are.
//
// A scenario harness — clusters broken in known ways, investigated through the real product
// and scored by a human — once shared this package's provisioning. It was retired with the
// evidence-chain investigation model it exercised; whether an investigation-scoring harness
// returns is decided against the provenance model that replaces it.
//
...
//     protocol intact, so what an investigation records about a masked field is read from
