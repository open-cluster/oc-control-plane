...
//
//   - AN INVESTIGATION RUN END TO END. This is the one gap that matters, because
//     plans/spec-first-investigation.md names this harness as the seam that proves it: a real
//     cluster is broken, an investigation runs against it through the real protocol, and the
//     conclusion is asserted against durable state. The whole path exists and is proven at the
//     composition root against a programmable relay — the case, its rounds, the brief, the truth
//     chain, the outcome and every read model — and none of that has met a real cluster.
//
//     What it needs, so the next person does not have to work it out. The control plane runs here
//     as a child process, so its model boundary has to arrive by configuration: that is
//     OC_MODEL_TRANSCRIPT_FILE, which exists and is tested. The operator surface has to be bound
//     and given a token, which is two more environment values. A Kubernetes Connection has to be
//     created through that surface, and an investigation opened against the crashing fixture this
//     harness already builds. The one genuine obstacle is the recorded transcript: it proposes a
//     log read naming a POD, and a Deployment's pod name is generated by the cluster. Either the
//     fixture becomes a StatefulSet, whose pod is deterministically <name>-0, or the harness reads
//     the pod name it already reads for the log capability and writes the transcript file before
//     starting the control plane.
//
//     It is recorded here rather than left silent because the specification requires this list to
//     be exhaustive, and a harness that ran green while quietly covering less than it appeared to
