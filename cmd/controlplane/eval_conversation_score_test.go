...
// has been compacted, must still be there. This is the assertion that makes memory loss a
...
func TestASurvivingFactIsFoundInTheLastTurnThatCouldCarryIt(t *testing.T) {
	...
	if !score.CompactionHappened {
		t.Errorf("compactions = %d; the world exists to force one", record.Compactions)
	}
}
...
func TestAFactPresentOnlyBeforeCompactionIsNotSurviving(t *testing.T) {
	t.Parallel()
	...
	one := evalCase{
		...
		FollowUps: []string{"and the cache?", "what did we establish about the deploy?"},
		Truth:     groundTruth{Survives: []string{"abc123"}, ExpectCompaction: true},
	}
	record := evalRecord{
		Status:      "concluded",
		Compactions: 1,
...
// A case the harness expected to compact and which did not is a case whose result says
// nothing about memory. It is reported rather than quietly passing.
func TestACaseThatNeverCompactedIsReportedAsSuch(t *testing.T) {
	t.Parallel()
	...
	record := evalRecord{Status: "concluded", Compactions: 0, Turns: []evalTurn{
		{Turn: 1, Answer: "commit abc123 raised the pool timeout."},
	}}
	...
	if score.CompactionHappened {
		t.Errorf("the score claims a compaction the record never saw")
	}
	if !score.CompactionExpected {
		t.Errorf("the score forgot that this world exists to force one")
	}
}
